...
/**
 * The certificate in `/certs/localhost-server.crt` carries a single `DNS:localhost` SAN, so
 * reaching the same server as `127.0.0.1` is a hostname mismatch and nothing else.
 */
class HostnameVerificationSpec
  ...
  private def settingsFor(host: String, verifyHostname: Boolean): GrpcClientSettings =
    GrpcClientSettings
  ...
  private def handshake(host: String, verifyHostname: Boolean): Try[HttpResponse] = {
    val context = PekkoHttpClientUtils.connectionContext(settingsFor(host, verifyHostname), system.log)
  ...
  "The pekko-http client connection context" should {
    ...
    "accept a certificate matching the requested hostname when verification is on" in {
    ...
    "reject a certificate that does not match the requested hostname when verification is on" in {
      // this is the regression guard: with endpoint identification unset the handshake below
    ...
    "accept a certificate that does not match the requested hostname when verification is off" in {
  ...
  "insecureSslEngineCreator" should {
    ...
    "leave endpoint identification unset" in {
      val engine = PekkoHttpClientUtils.insecureSslEngineCreator(SSLContext.getDefault)("example.com", 443)

      // null or empty both mean "do not check"; the JDK returns null when never set
      Option(engine.getSSLParameters.getEndpointIdentificationAlgorithm).getOrElse("") shouldBe ""
    }
